...
fun AnnualScreen() {
    ...
    var selectedLeave by remember { mutableStateOf(AnnualLeave()) }
    var selectedYear by remember { mutableIntStateOf(today.year) }
    var expandedYear by remember { mutableStateOf(false) }
    val optionsYear =
        viewModel.annualLeavesYears().collectAsStateWithLifecycle(initialValue = emptyList())

    val totalAnnualLeaves =
        viewModel.getTotalAnnualLeavesPerYear(selectedYear.toString())
            .collectAsStateWithLifecycle(initialValue = 20)

    val leaves by viewModel.getAnnualLeaves(selectedYear.toString())
        .map { leaves -> leaves.filterIsInstance<AnnualLeave>() }
    ...
    Column(
                    ...
                    if (leaves.isNotEmpty()) {
                        ...
                        item {
                            Column(
                                ...
                                Icon(
                                    Icons.Default.Info,
                                    contentDescription = stringResource(id = R.string.no_annual_leaves),
                                    modifier = Modifier
                                )
                                Text(
                                    text = stringResource(id = R.string.no_annual_leaves)
                                )
                            }
                        }
                    }
        ...
        AnnualSettingsSheet(year = selectedYear, bottomSheetState = settingsSheetState)
        NewAnnualLeaveSheet(year = selectedYear, bottomSheetState = newLeaveSheetState)
        EditAnnualLeaveSheet(
            year = selectedYear,
            leave = selectedLeave,
